...
fun TabTransactionEditor(
    modifier: Modifier = Modifier,
    viewModel: TabsViewModel,
    tabID: String,
    navController: NavController
){
    Log.d("TabTransactionEditor", "tabID: $tabID")

    var showDatePicker by remember { mutableStateOf(false) }

    LaunchedEffect(tabID) {
        if (tabID.isNotEmpty() && tabID != "Default ID") { // Avoid fetching with a default or empty ID
            viewModel.fetchTab(tabID)
            Log.d("TabTransactionEditor", "Calling fetchTab for: $tabID")
        }
        else{
...
@Preview
@Composable
fun TabTransactionEditorPreview(){
    CoinTrailTheme {
